...
class Youden(QualityMeasure):
    ...
    def __new__(cls) -> 'Youden':
        if Youden._singleton is None:
            Youden._singleton = object().__new__(cls)
        return Youden._singleton
    ...
    def compute(self, dict_of_parameters : dict[str, Union[int, float]]) -> float:
        ...
        :return: the computed value for the Youden quality measure.
        """
        ...
        return (tp/TP) + ((FP-fp)/FP) - 1 # Sensitivity + Specificity - 1
    ...
    def __call__(self, dict_of_parameters : dict[str, Union[int, float]]) -> float:
        """Compute the Youden quality measure.
